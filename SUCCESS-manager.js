...
const KEYS = {
    ...
    DAILY_ADDED: 'rulate_daily_added_count'
};
...
style.innerHTML = `
    ...
    .r-missing-card.overdue-severe { border-color: #dc3545 !important; background: rgba(220, 53, 69, 0.4) !important; box-shadow: inset 0 0 10px rgba(220,53,69,0.5); }

    .r-toast { position: fixed; top: 30px; left: 50%; transform: translate(-50%, -20px); background: #28a745; color: white; padding: 10px 20px; border-radius: 8px; z-index: 100000; font-weight: bold; box-shadow: 0 5px 15px rgba(0,0,0,0.5); opacity: 0; transition: 0.3s; pointer-events: none; font-family: sans-serif; }
    ...
    .r-book-rem-wrapper { display: inline-flex; align-items: center; margin-left: 15px; vertical-align: middle; gap: 10px; background: rgba(0,0,0,0.05); padding: 5px 10px; border-radius: 5px; border: 1px solid #ddd; }
    html[data-theme="dark"] .r-book-rem-wrapper { background: rgba(255,255,255,0.05); border-color: #444; }
    .r-book-rem-text { font-size: 13px; font-weight: bold; color: #666; font-family: sans-serif; }
    html[data-theme="dark"] .r-book-rem-text { color: #aaa; }
    .r-book-rem-text.overdue { color: #dc3545; animation: r-blink 1.5s infinite; text-shadow: 0 0 5px rgba(220,53,69,0.5); }
    @keyframes r-blink { 0%, 100% { opacity: 1; } 50% { opacity: 0.4; } }

    /* СТИЛИ МОДАЛЬНОГО ОКНА */
    .r-book-rem-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.6); z-index: 100004; display: none; backdrop-filter: blur(3px); }
    .r-book-rem-modal { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: #212529; border: 1px solid #444; border-radius: 10px; padding: 20px; z-index: 100005; box-shadow: 0 10px 30px rgba(0,0,0,0.8); color: white; display: none; width: 320px; font-family: sans-serif;}
...
menuDiv.innerHTML = `
    <!-- НАДПИСЬ SUCCESS MANAGER -->
    <div style="position: absolute; top: -26px; right: 10px; background: rgba(30, 30, 35, 0.95); padding: 4px 15px; border-radius: 8px 8px 0 0; color: #ffd700; font-weight: bold; border: 1px solid #444; border-bottom: none; z-index: -1; font-size: 12px; letter-spacing: 1px; box-shadow: 0 -5px 10px rgba(0,0,0,0.3);">
        SUCCESS MANAGER
    </div>
    ...
    <div class="r-content" id="content8">
        ...
        <div class="r-list" style="height:320px; max-height:320px;">
            ...
            <div id="rem_list_hold" style="margin-top:10px;"></div>
        </div>
    </div>
    ...
    <div class="r-content" id="content2">
        ...
        <!-- НОВЫЙ БЛОК: Проверка заброшек в профиле -->
        <div style="background:rgba(0,0,0,0.2); padding:10px; border-radius:5px; margin-bottom:15px; margin-top:10px; border-left: 3px solid #ffc107;">
            <div style="margin-bottom:5px;font-weight:bold;color:#ffc107;">Проверка заброшек профиля:</div>
            <div style="display:flex; gap:5px;">
                <input type="number" id="inp_check_user_id" class="r-input" placeholder="ID (напр. 554391)">
                <button id="btn_check_profile" class="r-btn" style="background:#007bff; width:auto; padding:8px 15px; margin:0;">Проверить</button>
            </div>
            <div style="font-size:10px; color:#aaa; margin-top:5px;">Ищет "Заброшен" и риск заброса (1-7 дней).</div>
        </div>
    ...
    <!-- ВЫЕЗЖАЮЩАЯ ПАНЕЛЬ ДЛЯ НАПОМИНАНИЙ -->
    <div class="r-side-drawer" id="missing_rem_drawer">
      ...
      <div class="r-side-drawer" id="profile_check_drawer">
        <div class="r-drawer-header">
            <span>Анализ заброшек</span>
            <button id="btn_close_profile_drawer" style="background:none; border:none; color:#fff; cursor:pointer; font-size:16px;">✖</button>
        </div>
        <div style="padding: 5px 10px; background: rgba(0,0,0,0.2); border-bottom: 1px solid #444; font-size:11px;">
            <label style="display:flex; align-items:center; cursor:pointer; color:#aaa; margin-bottom:3px;">
                <input type="checkbox" id="chk_hide_completed" checked style="margin-right:5px;"> Скрыть "Завершён"
            </label>
            <label style="display:flex; align-items:center; cursor:pointer; color:#aaa;">
                <input type="checkbox" id="chk_hide_hiatus" checked style="margin-right:5px;"> Скрыть "Перерыв"
            </label>
        </div>
        <div class="r-drawer-content" id="profile_check_list">
            <div style="text-align:center; color:#aaa; margin-top:20px;">Введите ID и нажмите Проверить</div>
        </div>
    </div>
`;
...
menuDiv.addEventListener('click', (e) => {
    ...
    if (actBtn) {
        ...
        if (action === 'rem_pause') {
            reminders[idx].status = 'on_hold';
            localStorage.setItem(KEYS.REMINDERS, JSON.stringify(reminders));
            renderReminders();
            return;
        }
        if (action === 'rem_resume') {
            reminders[idx].status = 'scheduled';
            reminders[idx].time = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
            localStorage.setItem(KEYS.REMINDERS, JSON.stringify(reminders));
            renderReminders();
            return;
        }
...
function renderMissingReminders() {
    ...
    missing.forEach(b => {
        ...
        const addedTime = b.dateAdded || 0;
        const daysPassed = addedTime === 0 ? 999 : Math.floor((Date.now() - addedTime) / 86400000);

        let overdueText = '';
        if (daysPassed > 6) {
            d.classList.add('overdue-severe');
            overdueText = addedTime === 0
                ? `<span style="color:#ffcccc; font-weight:bold; margin-left:5px;">(Добавлена давно)</span>`
                : `<span style="color:#ffcccc; font-weight:bold; margin-left:5px;">(Без напом. ${daysPassed} дн.)</span>`;
        }
        const dateStrText = addedTime === 0 ? 'Неизвестно' : new Date(addedTime).toLocaleDateString();
...
function renderReminders() {
    ...
    let fixNeeded = false;
    reminders.forEach(r => {
        // Если задача просрочена хотя бы на 1 минуту, жестко переводим её в "Ожидают"
        if (r.status === 'scheduled' && (now - new Date(r.time).getTime() > 60000)) {
            r.status = 'pending';
            fixNeeded = true;
        }
    });
    if (fixNeeded) localStorage.setItem(KEYS.REMINDERS, JSON.stringify(reminders));
    ...
    const holdContainer = document.getElementById('rem_list_hold');
    schedContainer.innerHTML = ''; waitContainer.innerHTML = ''; holdContainer.innerHTML = '';
    ...
    const onHold = reminders.filter(r => r.status === 'on_hold' && filterFn(r)).sort((a,b) => new Date(b.time) - new Date(a.time));

    if (waiting.length) {
        waitContainer.innerHTML = '<div class="r-rem-header">⚠️ Ожидают выполнения</div>';
        waiting.forEach((r) => {
             const realIdx = reminders.findIndex(x => x.id === r.id);
             let contentHtml = r.customText
                ? `<div style="display: flex; align-items: center;"><span style="margin-right: 5px;">📌</span><span class="r-rem-name" style="color: #fff;" title="${r.customText}">${r.customText}</span></div>`
                : `<div style="display: flex; align-items: center;"><a href="/book/${r.bookId}" target="_blank" class="r-rem-name" title="${r.bookName}">${r.bookName}</a></div>`;

             const daysLate = Math.floor((Date.now() - new Date(r.time).getTime()) / 86400000);
             ...
             const chapInfo = `<span class="rem-chap-container" style="font-size:10px; color:#17a2b8; margin-left: 5px;">[Главы: ${chapText}] <span class="r-act-btn" style="color:#aaa; padding:0 2px; font-size:10px;" data-action="rem_edit_chaps" data-index="${realIdx}">✏️</span></span>`;

             const d = document.createElement('div');
             d.className = 'r-rem-card wait' + severeClass;
             d.innerHTML = `
                 <div style="flex:1; overflow:hidden; display:flex; flex-direction:column;">${contentHtml}<div style="font-size:10px; color:#aaa; margin-top:2px;">Сработало: ${new Date(r.time).toLocaleTimeString()} ${lateText} ${chapInfo}</div></div>
                 <div class="r-rem-act">
                    <span class="r-act-btn" style="color:#ffc107;" data-action="rem_edit_time" data-index="${realIdx}" title="Перенести дату">📅</span>
                    <span class="r-act-btn" style="color: #6c757d;" data-action="rem_pause" data-index="${realIdx}" title="В ожидание (заморозить)">⏸️</span>
    ...
    if (scheduled.length) {
        ...
        scheduled.forEach((r) => {
            ...
            if (dStr !== lastDate) {
                ...
                if (dStr === new Date().toDateString()) headerText = '📅 Сегодня';
                else if (dStr === new Date(Date.now() + 86400000).toDateString()) headerText = '📆 Завтра';

                const h = document.createElement('div'); h.className = 'r-rem-header'; h.style.marginTop = '10px'; h.innerText = headerText;
                schedContainer.appendChild(h); lastDate = dStr;
            }

            const realIdx = reminders.findIndex(x => x.id === r.id);
            let contentHtml = r.customText ? `<span style="margin-right: 5px;">📌</span><span class="r-rem-name" style="color:#fff;">${r.customText}</span>` : `<a href="/book/${r.bookId}" target="_blank" class="r-rem-name">${r.bookName}</a>`;
            const chapText = r.chapters === 'auto' ? 'По мере выхода' : (r.chapters || '0');

            const div = document.createElement('div'); div.className = 'r-rem-card';
            div.innerHTML = `
                <div class="r-rem-time">${dObj.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div>
                <div style="flex:1; overflow:hidden; display:flex; flex-direction:column;">
                    <div style="display:flex; align-items:center;">${contentHtml}</div>
                    <div class="rem-chap-container" style="font-size:10px; color:#17a2b8;">Главы: ${chapText} <span class="r-act-btn" style="color:#aaa; padding:0 2px; font-size:10px;" data-action="rem_edit_chaps" data-index="${realIdx}">✏️</span></div>
                </div>
                <div class="r-rem-act">
                    <span class="r-act-btn" style="color:#ffc107;" data-action="rem_edit_time" data-index="${realIdx}" title="Перенести">📅</span>
                    <span class="r-act-btn" style="color: #6c757d;" data-action="rem_pause" data-index="${realIdx}" title="В ожидание">⏸️</span>
    ...
    if (onHold.length) {
        holdContainer.innerHTML = '<div class="r-rem-header" style="color:#6c757d; border-color:#444; margin-top:15px;">❄️ Ожидание глав (Пауза)</div>';
        onHold.forEach((r) => {
            const realIdx = reminders.findIndex(x => x.id === r.id);
            let contentHtml = r.customText ? `<span style="margin-right: 5px; opacity: 0.5;">📌</span><span class="r-rem-name" style="color:#888;">${r.customText}</span>` : `<a href="/book/${r.bookId}" target="_blank" class="r-rem-name" style="color:#888;">${r.bookName}</a>`;

            const div = document.createElement('div');
            div.className = 'r-rem-card';
            div.style.backgroundColor = 'rgba(0,0,0,0.4)';
            div.style.borderLeftColor = '#444';
            div.innerHTML = `
                <div style="flex:1; overflow:hidden; display:flex; flex-direction:column;">
                    <div style="display:flex; align-items:center;">${contentHtml}</div>
                    <div style="font-size:10px; color:#666;">На паузе. Глав: ${r.chapters === 'auto' ? 'auto' : (r.chapters||'0')}</div>
                </div>
                <div class="r-rem-act">
                    <span class="r-act-btn" style="color: #28a745;" data-action="rem_resume" data-index="${realIdx}" title="Возобновить (перенесет на завтра)">▶️</span>
                    <span class="r-act-btn r-act-del" data-action="rem_del" data-index="${realIdx}" title="Удалить">✖</span>
                </div>`;
    ...
    if (!waiting.length && !scheduled.length && !onHold.length) {
...
toastDiv.className = 'r-toast';
document.body.appendChild(toastDiv);

function showAddToast() {
    let data = JSON.parse(localStorage.getItem(KEYS.DAILY_ADDED) || '{"date":"","count":0}');
    const today = new Date().toDateString();
    if (data.date !== today) { data.date = today; data.count = 0; }
    data.count++;
    localStorage.setItem(KEYS.DAILY_ADDED, JSON.stringify(data));

    toastDiv.innerText = `Добавлено сегодня: ${data.count} шт.`;
...
setInterval(() => {
    ...
    reminders.forEach(r => {
        if (r.status === 'scheduled' && new Date(r.time).getTime() <= now) {
            if (reminderOpts.disableNotifications) {
                // Если уведомления отключены, просто тихо переводим задачу в долги
                r.status = 'pending';
                needsSave = true;
            } else {
                // Если включены - показываем всплывающее окно
    ...
    if (needsSave) {
        localStorage.setItem(KEYS.REMINDERS, JSON.stringify(reminders));
        // Если меню открыто на вкладке напоминаний — обновляем визуал в реальном времени
        if (isMenuOpen && document.getElementById('content8').classList.contains('active')) {
            renderReminders();
        }
    }
}, 5000);
...
document.getElementById('inp_import_file').onchange = (e) => {
 ...
 // ==========================================
// ЛОГИКА ПРОВЕРКИ ПРОФИЛЯ (БОТ - ЗАБРОШКИ)
// ==========================================

// Парсер дат Рулейта: "26.03.2026 04:00" -> Timestamp
function parseRulateDate(dateStr) {
    const m = dateStr.trim().match(/(\d{1,2})\.(\d{1,2})\.(\d{4})\s+(\d{1,2}):(\d{1,2})/);
    if (!m) return 0;
    return new Date(m[3], m[2] - 1, m[1], m[4], m[5]).getTime();
}

let profileAtRisk = [];
let profileAbandoned = [];

// Функция динамической отрисовки с учетом галочек
function renderProfileCheck() {
    const list = document.getElementById('profile_check_list');
    const hideCompleted = document.getElementById('chk_hide_completed').checked;
    const hideHiatus = document.getElementById('chk_hide_hiatus').checked;

    // Фильтруем массивы
    const filterFn = (b) => {
        if (hideCompleted && b.isCompleted) return false;
        if (hideHiatus && b.isHiatus) return false;
        return true;
    };

    const filteredAtRisk = profileAtRisk.filter(filterFn);
    const filteredAbandoned = profileAbandoned.filter(filterFn);

    let resHtml = '';

    if (filteredAtRisk.length > 0) {
        resHtml += `<div class="r-rem-header" style="color:#ffc107; border-color:#ffc107;">⚠️ Риск заброса (1-7 дней) - ${filteredAtRisk.length} шт.</div>`;
        filteredAtRisk.forEach(b => {
            const daysLeft = 60 - b.daysPassed;
            const statusBadge = b.isHiatus ? ' <span style="color:#17a2b8;">[Перерыв]</span>' : (b.isCompleted ? ' <span style="color:#28a745;">[Завершён]</span>' : '');
            resHtml += `
            <div class="r-missing-card wait" style="border-left-color:#ffc107; background:rgba(255, 193, 7, 0.1);">
                <div style="font-weight:bold;"><a href="${b.link}" target="_blank" style="color:#ffc107; text-decoration:none;">${b.name}</a>${statusBadge}</div>
                <div style="font-size:10px; color:#aaa; margin-top:3px;">Обновлено: ${b.dateStr} (прошло ${b.daysPassed} дн.)</div>
                <div style="font-size:10px; color:#dc3545; margin-top:2px; font-weight:bold;">В заброшки через: ~${daysLeft} дн.</div>
            </div>`;
        });
    }

    if (filteredAbandoned.length > 0) {
        resHtml += `<div class="r-rem-header" style="color:#dc3545; border-color:#dc3545; margin-top:15px;">💀 Уже заброшены (или > 2 мес.) - ${filteredAbandoned.length} шт.</div>`;
        filteredAbandoned.forEach(b => {
            const statusBadge = b.isHiatus ? ' <span style="color:#17a2b8;">[Перерыв]</span>' : (b.isCompleted ? ' <span style="color:#28a745;">[Завершён]</span>' : '');
            resHtml += `
            <div class="r-missing-card overdue-severe" style="border-left-color:#dc3545; background:rgba(220, 53, 69, 0.15);">
                <div style="font-weight:bold;"><a href="${b.link}" target="_blank" style="color:#ff6b6b; text-decoration:none;">${b.name}</a>${statusBadge}</div>
                <div style="font-size:10px; color:#ccc; margin-top:3px;">Обновлено: ${b.dateStr} (прошло ${b.daysPassed} дн.)</div>
            </div>`;
        });
    }

    // Если всё пусто
    if (filteredAtRisk.length === 0 && filteredAbandoned.length === 0) {
        if (profileAtRisk.length === 0 && profileAbandoned.length === 0) {
             resHtml = '<div style="text-align:center; color:#28a745; margin-top:30px; font-weight:bold; font-size:14px;">Всё отлично!<br>Рисков и заброшек нет 🎉</div>';
        } else {
             resHtml = '<div style="text-align:center; color:#aaa; margin-top:30px; font-size:12px;">Книги есть, но они скрыты фильтрами.</div>';
        }
    }
...
// Привязываем события на переключение чекбоксов
document.getElementById('chk_hide_completed').addEventListener('change', renderProfileCheck);
document.getElementById('chk_hide_hiatus').addEventListener('change', renderProfileCheck);

// Основная функция запуска
document.getElementById('btn_check_profile').onclick = async () => {
    const uid = document.getElementById('inp_check_user_id').value.trim();
    if (!uid) return alert('Введите ID пользователя');

    const drawer = document.getElementById('profile_check_drawer');
    const list = document.getElementById('profile_check_list');
    drawer.classList.add('open');
    list.innerHTML = '<div style="text-align:center; margin-top:30px;"><div class="r-pulse-icon" style="font-size:40px;">⏳</div><div>Идет сканирование профиля...</div></div>';

    // Принудительно включаем компактный вид для профилей (через cookie сайта)
    document.cookie = "books_view_mode=compact; path=/; max-age=3600";

    let currentPage = 1;
    let maxPage = 1;

    // Очищаем массивы перед новым поиском
    profileAbandoned = [];
    profileAtRisk = [];
    ...
    try {
        while (currentPage <= maxPage) {
            list.innerHTML = `<div style="text-align:center; margin-top:30px;"><div class="r-pulse-icon" style="font-size:40px;">⏳</div><div>Сканирование страницы ${currentPage} из ${maxPage}...</div></div>`;

            const url = `/users/${uid}?GroupMember_page=${currentPage}`;
            const resp = await fetch(url);
            const html = await resp.text();
            const parser = new DOMParser();
            const doc = parser.parseFromString(html, "text/html");

            // Ищем количество страниц при первом проходе
            if (currentPage === 1) {
                const lastPageLink = doc.querySelector('.yiiPager li.last a');
                if (lastPageLink) {
                    const match = lastPageLink.href.match(/GroupMember_page=(\d+)/);
                    if (match) maxPage = parseInt(match[1]);
                } else {
                    const pageLinks = doc.querySelectorAll('.yiiPager li a');
                    pageLinks.forEach(a => {
                        const m = a.href.match(/GroupMember_page=(\d+)/);
                        if (m && parseInt(m[1]) > maxPage) maxPage = parseInt(m[1]);
                    });
                }
            }

            // Ищем строки в компактной таблице
            const rows = doc.querySelectorAll('table.books-compact-table tbody tr');
            if (rows.length === 0 && currentPage === 1) {
                list.innerHTML = '<div style="text-align:center; color:#dc3545; margin-top:20px;">Книги не найдены или закрыт профиль. Убедитесь, что ID правильный.</div>';
                return;
            }

            rows.forEach(row => {
                const titleEl = row.querySelector('td[data-label="Название"] a');
                const statusEl = row.querySelector('td[data-label="Статус"]');
                const dateEl = row.querySelector('td[data-label="Обновлено"]');

                if (titleEl && statusEl && dateEl) {
                    const name = titleEl.innerText.trim();
                    const link = titleEl.getAttribute('href');
                    const statusText = statusEl.innerText.trim().toLowerCase();
                    const dateStr = dateEl.innerText.trim();

                    const timestamp = parseRulateDate(dateStr);
                    if (timestamp > 0) {
                        const daysPassed = Math.floor((now - timestamp) / DAY_MS);

                        const isCompleted = statusText.includes('завершён') || statusText.includes('завершен');
                        const isHiatus = statusText.includes('перерыв');
                        const isAbandonedStatus = statusText.includes('брошен') || statusText.includes('заброшен');

                        const bookData = { name, link, daysPassed, dateStr, statusText, isCompleted, isHiatus, isAbandonedStatus };

                        // Логика распределения:
                        if (isAbandonedStatus) {
                            profileAbandoned.push(bookData);
                        }
                        // Если статус не "заброшен", но время (60 дней) вышло
                        else if (daysPassed >= 60) {
                            profileAbandoned.push(bookData);
                        }
                        // Если время подходит к концу (1-7 дней)
                        else if (daysPassed >= 53) {
                            profileAtRisk.push(bookData);
                        }
                    }
                }
            });
            ...
            currentPage++;
            // Небольшая задержка, чтобы не ловить блокировки
            await new Promise(r => setTimeout(r, 600));
        }

        // Отрисовываем результаты после сбора всех книг
        renderProfileCheck();

    } catch(e) {
        console.error("Ошибка парсинга профиля:", e);
        list.innerHTML = `<div style="text-align:center; color:#dc3545; margin-top:20px;">Ошибка при парсинге профиля. Попробуйте перезагрузить страницу.</div>`;
    }
};

document.getElementById('btn_close_profile_drawer').onclick = () => {
    document.getElementById('profile_check_drawer').classList.remove('open');
};


// ==========================================
// 7. ИНТЕГРАЦИЯ НА СТРАНИЦУ КНИГИ (КНОПКА И МОДАЛКА)
// ==========================================
function initBookPageReminder() {
    const toolbar = document.querySelector('.btn-toolbar.btn-toolbar-custom');
    if (!currentBookId || !toolbar) return; // Если мы не на странице книги, выходим

    // 1. Создаем контейнер рядом с кнопками
    const wrapper = document.createElement('div');
    wrapper.className = 'r-book-rem-wrapper';
    wrapper.innerHTML = `
        <button class="btn btn-success" id="btn_page_add_success" style="margin:0; padding: 4px 10px; display: none;">➕ В SUCCESS</button>
        <button class="btn btn-info" id="btn_page_rem_edit" style="margin:0; padding: 4px 10px; display: none;">⏰ Напоминание</button>
        <button class="btn btn-warning" id="btn_page_finish" style="margin-left:5px; padding: 4px 10px; display: none; color: #000; font-weight: bold;">🏁 Завершить</button>
        <span id="page_rem_status" class="r-book-rem-text">Загрузка...</span>
    `;
    toolbar.appendChild(wrapper);

    // 2. Создаем модальное окно
    const modalOverlay = document.createElement('div');
    modalOverlay.className = 'r-book-rem-overlay';
    const modalBox = document.createElement('div');
    modalBox.className = 'r-book-rem-modal';
    modalBox.innerHTML = `
        <h4 style="margin-top:0; color:#61dafb; font-size: 16px; margin-bottom: 15px;">Изменить дату напоминания</h4>
        <label style="font-size: 12px; color: #ccc;">Дата и время:</label>
        <input type="datetime-local" id="inp_page_rem_date" class="r-input" style="width:100%; margin-bottom:10px;">
        <label style="font-size: 12px; color: #ccc;">Кол-во глав (или auto):</label>
        <input type="text" id="inp_page_rem_chaps" class="r-input" placeholder="Напр. 10 или auto" style="width:100%; margin-bottom:15px;">
        <div style="display:flex; gap:10px;">
            <button class="r-btn" id="btn_page_rem_save" style="background:#28a745; margin:0; flex: 1;">💾 Сохранить</button>
            <button class="r-btn" id="btn_page_rem_cancel" style="background:#6c757d; margin:0; flex: 1;">✖ Отмена</button>
        </div>
        <button class="r-btn" id="btn_page_rem_delete" style="background:#dc3545; margin-top:10px; display:none;">🗑️ Удалить напоминание</button>
    `;
    document.body.appendChild(modalOverlay);
    document.body.appendChild(modalBox);

    // Вспомогательная функция формата времени
    function formatTimeDiff(ms) {
        ...
        if (h > 0 || d > 0) str += `${h}ч `;
        str += `${m}м`;
        return { str: str || '< 1м', isNegative };
    }

    // 3. Функция обновления статуса (текст и таймер)
    function updateStatusText() {
        const statusEl = document.getElementById('page_rem_status');
        const btnRemEl = document.getElementById('btn_page_rem_edit');
        const btnAddEl = document.getElementById('btn_page_add_success');
        const btnFinishEl = document.getElementById('btn_page_finish');

        if (!statusEl || !btnRemEl || !btnAddEl || !btnFinishEl) return;

        const currentBook = savedBooks.find(b => String(b.id) === String(currentBookId));

        if (!currentBook) {
            // Книги нет в менеджере
            btnAddEl.style.display = 'inline-block';
            btnRemEl.style.display = 'none';
            btnFinishEl.style.display = 'none';
            statusEl.innerText = 'Книги нет в менеджере';
            statusEl.className = 'r-book-rem-text';
            statusEl.style.color = '';
            return;
        }

        // Книга есть в менеджере
        btnAddEl.style.display = 'none';
        btnFinishEl.style.display = 'inline-block';

        // Проверяем, завершена ли книга
        if (currentBook.status === 'finished') {
            btnFinishEl.innerText = '🟢 Возобновить';
            btnFinishEl.className = 'btn btn-default';
            btnFinishEl.style.color = '#333';

            btnRemEl.style.display = 'none'; // Скрываем напоминания для завершенных

            statusEl.innerText = '🏁 Книга завершена';
            statusEl.className = 'r-book-rem-text';
        ...
        // Книга в процессе
        btnFinishEl.innerText = '🏁 Завершить';
        btnFinishEl.className = 'btn btn-warning';
        btnFinishEl.style.color = '#000';
        btnRemEl.style.display = 'inline-block';
        statusEl.style.color = '';
        statusEl.style.textDecoration = 'none';

        // Ищем напоминание для текущей книги
        const rem = reminders.find(r => String(r.bookId) === String(currentBookId));

        if (!rem || rem.status === 'finished') {
            let daysPassedText = '';
            let isSevere = false;

            const addedTime = currentBook.dateAdded || 0;
            const daysPassed = addedTime === 0 ? 999 : Math.floor((Date.now() - addedTime) / 86400000);

            if (daysPassed > 6) {
                isSevere = true;
                daysPassedText = addedTime === 0 ? '(Добавлена давно)' : `(Без напом. ${daysPassed} дн.)`;
            }

            if (isSevere) {
                statusEl.innerHTML = `Нет напоминания <b>${daysPassedText}</b>`;
                statusEl.className = 'r-book-rem-text overdue';
                btnRemEl.className = 'btn btn-danger';
            } else {
                statusEl.innerText = 'Напоминание не задано';
                statusEl.className = 'r-book-rem-text';
                btnRemEl.className = 'btn btn-success';
            }

            btnRemEl.innerText = '⏰ Добавить';
            return;
        }

        btnRemEl.innerText = '⏰ Изменить';
        btnRemEl.className = 'btn btn-info';

        if (rem.status === 'on_hold') {
            statusEl.innerText = '⏸ На паузе';
            statusEl.className = 'r-book-rem-text';
            return;
        }

        const targetTime = new Date(rem.time).getTime();
        const diff = targetTime - Date.now();
        const { str, isNegative } = formatTimeDiff(diff);

        if (isNegative) {
            statusEl.innerHTML = `Просрочено на: <b>${str}</b>`;
            statusEl.className = 'r-book-rem-text overdue';
        } else {
            ...
            statusEl.innerHTML = `До ${dateStr} <span style="font-weight:normal;">(осталось ${str})</span>`;
            statusEl.className = 'r-book-rem-text';
        }
    }
    ...
    // --- ОБРАБОТЧИК ДЛЯ КНОПКИ "➕ В SUCCESS" ---
    document.getElementById('btn_page_add_success').addEventListener('click', (e) => {
        e.preventDefault();
        let bookName = `Book #${currentBookId}`;
        const titleNode = document.querySelector('h1');
        if (titleNode) {
            const clone = titleNode.cloneNode(true);
            if (clone.querySelector('small')) clone.querySelector('small').remove();
            bookName = clone.innerText.trim();
        }

        savedBooks.push({id: currentBookId, name: bookName, status: 'process', originalUrl: '', dateAdded: Date.now()});
        localStorage.setItem(KEYS.BOOKS, JSON.stringify(savedBooks));
    ...
    // --- ОБРАБОТЧИК ДЛЯ КНОПКИ "🏁 ЗАВЕРШИТЬ" / "🟢 ВОЗОБНОВИТЬ" ---
    document.getElementById('btn_page_finish').addEventListener('click', (e) => {
        e.preventDefault();
        const bookIdx = savedBooks.findIndex(b => String(b.id) === String(currentBookId));
        if (bookIdx === -1) return;

        const isFinished = savedBooks[bookIdx].status === 'finished';

        if (isFinished) {
            // Возобновляем книгу
            savedBooks[bookIdx].status = 'process';
        } else {
            // Завершаем книгу
            if (!confirm('Отметить книгу как завершенную? Все её напоминания будут удалены.')) return;

            savedBooks[bookIdx].status = 'finished';

            // Удаляем напоминание, если оно есть
            const remIdx = reminders.findIndex(r => String(r.bookId) === String(currentBookId));
            if (remIdx !== -1) {
        ...
        // Обновляем UI на странице книги
        updateStatusText();
    });

    // 4. Логика кнопок модалки
    const openModal = () => {
        const rem = reminders.find(r => String(r.bookId) === String(currentBookId));
        if (rem) {
            const d = new Date(rem.time);
            d.setMinutes(d.getMinutes() - d.getTimezoneOffset());
            document.getElementById('inp_page_rem_date').value = d.toISOString().slice(0,16);
            document.getElementById('inp_page_rem_chaps').value = rem.chapters || '';
            document.getElementById('btn_page_rem_delete').style.display = 'block';
        } else {
            const tomorrow = new Date(Date.now() + 86400000);
            tomorrow.setMinutes(tomorrow.getMinutes() - tomorrow.getTimezoneOffset());
            document.getElementById('inp_page_rem_date').value = tomorrow.toISOString().slice(0,16);
            document.getElementById('inp_page_rem_chaps').value = 'auto';
            document.getElementById('btn_page_rem_delete').style.display = 'none';
        }
    ...
    document.getElementById('btn_page_rem_edit').addEventListener('click', (e) => { e.preventDefault(); openModal(); });
    document.getElementById('btn_page_rem_cancel').addEventListener('click', closeModal);
    modalOverlay.addEventListener('click', closeModal);

    // Сохранение
    document.getElementById('btn_page_rem_save').addEventListener('click', () => {
        const dateVal = document.getElementById('inp_page_rem_date').value;
        const chapsVal = document.getElementById('inp_page_rem_chaps').value.trim().toLowerCase();

        if (!dateVal) return alert('Укажите дату!');

        let finalChaps = 0;
        if (chapsVal === 'auto' || chapsVal === 'ауто' || chapsVal === 'авто') finalChaps = 'auto';
        else if (parseInt(chapsVal) > 0) finalChaps = parseInt(chapsVal);

        let remIdx = reminders.findIndex(r => String(r.bookId) === String(currentBookId));

        if (remIdx !== -1) {
            reminders[remIdx].time = new Date(dateVal).toISOString();
            reminders[remIdx].chapters = finalChaps;
            reminders[remIdx].status = 'scheduled';
        } else {
            let bookName = `Book #${currentBookId}`;
            const titleNode = document.querySelector('h1');
            if (titleNode) {
    ...
    // Удаление
    document.getElementById('btn_page_rem_delete').addEventListener('click', () => {
        if (!confirm('Удалить напоминание для этой книги?')) return;
        const remIdx = reminders.findIndex(r => String(r.bookId) === String(currentBookId));
        if (remIdx !== -1) {
            reminders.splice(remIdx, 1);
            localStorage.setItem(KEYS.REMINDERS, JSON.stringify(reminders));
...
// Запускаем инициализацию UI на странице книги
initBookPageReminder();
