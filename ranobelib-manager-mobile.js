// ==UserScript==
// @name         RanobeLib Manager (Mobile Friendly)
// @namespace    http://tampermonkey.net/
// @version      7.3-mobile
// @description  Загрузка глав, очередь, платные главы, объединение (Merge), наценка за объем, статус на кнопке. Адаптировано для мобильных.
// @author       You
// @match        *://ranobelib.me/*
// @grant        GM_addStyle
...
(function() {
    ...
    const UI_THEME_KEY = 'rl_v7_theme';

    // --- CSS (Адаптированный для мобильных) ---
    GM_addStyle(`
        :root {
            --rl-bg: #1e1e1e;
            --rl-bg-header: #2d2d2d;
        ...
        /* --- Input --- */
        #rl-input {
            width: 100%; height: 120px; background: var(--rl-input-bg);
            border: 1px solid var(--rl-border); border-radius: 6px;
        ...
        .rl-del-btn { color: var(--rl-error); cursor: pointer; padding: 2px 6px; font-weight: bold; font-size: 18px; opacity: 0.7; }
        ...
        .rl-btn { padding: 10px 16px; border-radius: 6px; border: none; font-weight: 500; cursor: pointer; transition: transform 0.1s; font-size: 14px; -webkit-tap-highlight-color: transparent; }
    ...
    const uiHTML = `
            ...
            <div id="rl-window">
                <div class="rl-header">
                    ...
                    <div class="rl-win-controls">
                        ...
                        <button class="rl-icon-btn" id="rl-close-btn" title="Закрыть">✖️</button>
                    </div>
                </div>
                ...
                <div id="rl-settings">
                    <div class="rl-setting-row">
                        <span>Прозрачность UI:</span>
                        <input type="range" id="rl-opacity-slider" min="50" max="100" value="100">
                    </div>
                    <div class="rl-setting-row">
        ...
        <!-- REPORT PANEL (в корне, вне wrapper) -->
        <div id="rl-report-panel">
             <div class="rl-header">
                <div class="rl-title">📊 Отчет загрузки</div>
                <div class="rl-win-controls">
                    <button class="rl-icon-btn" id="rl-report-close-btn" title="Закрыть отчет">✖️</button>
                </div>
             </div>
    ...
    const closeReportBtn = document.getElementById('rl-report-close-btn');
    ...
    const closeBtn = document.getElementById('rl-close-btn');
    ...
    // --- UI ЛОГИКА ---
    const themeContainer = document.getElementById('rl-root-container');
    themeBtn.onclick = () => { themeContainer.classList.toggle('rl-light-theme'); localStorage.setItem(UI_THEME_KEY, themeContainer.classList.contains('rl-light-theme') ? 'light' : 'dark'); };
    if (localStorage.getItem(UI_THEME_KEY) === 'light') themeContainer.classList.add('rl-light-theme');

    floatBtn.onclick = () => { wrapper.style.display = 'flex'; };
    closeBtn.onclick = () => { wrapper.style.display = 'none'; };

    toggleReportBtn.onclick = () => {
        reportPanel.classList.toggle('open');
        toggleReportBtn.classList.toggle('active');
        if (reportPanel.classList.contains('open')) renderReport();
    };
    closeReportBtn.onclick = () => {
        reportPanel.classList.remove('open');
        toggleReportBtn.classList.remove('active');
    };
    ...
    function loadSettings() {
        ...
        if (saved) {
            state.settings = { ...state.settings, ...saved }; // Merge to avoid errors on new settings
            skipTitleChk.checked = state.settings.skipTitle;
            paidEnableChk.checked = state.settings.paidEnabled;
            paidPriceInput.value = state.settings.paidPrice || 5;
            paidBufferInput.value = state.settings.paidBuffer || 0;
            Array.from(mergeModeRadios).forEach(r => r.checked = (r.value === state.settings.mergeMode));
            mergePercentInput.value = state.settings.mergePercent || 50;
            mergeCharPriceInput.value = state.settings.mergeCharPrice || 2;
            mergeCharCountInput.value = state.settings.mergeCharCount || 2000;
            updateUI();
        }
    }
